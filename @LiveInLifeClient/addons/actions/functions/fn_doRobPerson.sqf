...
{
    [_target, "fingerprint", (player getVariable ["lilc_accountID", 0])] call lilc_proofs_fnc_addProof;
};

if ((headgear player) != "" && (random 1) < 0.1) then
{
    [_target, "hair", (player getVariable ["lilc_accountID", 0])] call lilc_proofs_fnc_addProof;
};

if ((random 1) < 0.2) then
{
    [_target, "scale", (player getVariable ["lilc_accountID", 0])] call lilc_proofs_fnc_addProof;
};

if ((damage player) > 0.4 && (random 1) < 0.5) then
{
    [_target, "blood", (player getVariable ["lilc_accountID", 0])] call lilc_proofs_fnc_addProof;
};

if ((damage _target) > 0.4 && (random 1 < 0.5)) then
{
    [player, "blood", (_target getVariable ["lilc_accountID", 0])] call lilc_proofs_fnc_addProof;
};
