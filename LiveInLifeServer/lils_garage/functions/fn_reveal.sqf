...
try
{
    ...
    {
        private _vehicleInfo = ([(format["SELECT CLASSNAME, GEAR, DAMAGE, FUEL, COLOR, QUOTE(PLATE) FROM VEHICLES_DATA WHERE ID = '%1' AND STEAM64ID = '%2' AND ACCOUNTID = '%3'", _vehicleID, _unitUID, _accountID])] call lils_database_fnc_fetchObjects);
