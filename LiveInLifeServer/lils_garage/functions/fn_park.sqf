...
try
{
    ...
    else
    {
        ...
        [(format["UPDATE VEHICLES_DATA SET ACTIVE = '0', GEAR = '%1' WHERE ID = '%2' AND ACCOUNTID = '%3' AND STEAM64ID = '%4' AND GARAGE = '""%5""' AND CLASSNAME = '""%6""';", ([([_vehicle] call lilc_inventory_fnc_getVehicleCargo)] call lils_common_fnc_arrayEncode),_vehicleID, _accountID, _unitUID, _garageName, _classname])] call lils_database_fnc_query;
