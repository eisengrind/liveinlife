...
//= TRANSID<string>, BANK<string>, AMOUNT<int>, STATUS<bool>, ID<int>
    ([(format["INSERT INTO BANK_ACCOUNT_DATA (ID, NAME, AMOUNT, ACCOUNTID, STEAM64ID, STATUS, HASH) VALUES (NULL, '""%1""', '%2', '%3', '%4', '%5', '%6')", _bankName, _startAmount, _accountID, _unitUID, 1, _hash])] call lils_database_fnc_query);
    private _bankAccount = ([(format["SELECT HASH, NAME, AMOUNT, STATUS, ID  FROM BANK_ACCOUNT_DATA WHERE ACCOUNTID = '%1' AND STEAM64ID = '%2' AND HASH = '%3'", _accountID, _unitUID, _hash])] call lils_database_fnc_fetchObjects);
