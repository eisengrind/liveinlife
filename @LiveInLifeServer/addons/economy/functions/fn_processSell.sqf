...
_sells = (((_dealerData select 0) select 3) select 0);
_sells set [0, (_sells + _amount)];

((((lils_economy_data select _itemIndex) select 1) select _dealerIndex) select 3) set [0, _sells];
