...
try {
    ...
    {
        if ((["canCreateNewProfile", "NUMBER", 1] call lilc_common_fnc_getSetting) != 1) then
        {
            throw false;
        }
        else
        {
            //if !(["signup"] call lilc_login_fnc_selectMenu) throw false;
        };
    }
    else
    {
        //if !(["login"] call lilc_login_fnc_selectMenu) throw false;
    };
    */
