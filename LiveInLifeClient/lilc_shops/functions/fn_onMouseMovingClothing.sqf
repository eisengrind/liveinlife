...
if (lilc_shops_clothing_isMouseDown) then {
    ...
    private _playerPositionHeight = (lilc_shops_clothing_defaultPosition select 2);
    if ((_playerPositionHeight + (_yDiff * 0.009)) > 2) then {
        lilc_shops_clothing_defaultPosition set [2, 2];
    } else {
        if ((_playerPositionHeight + (_yDiff * 0.009)) < 0.5) then {
            lilc_shops_clothing_defaultPosition set [2, 0.5];
        } else {
            lilc_shops_clothing_defaultPosition set [2, (_playerPositionHeight + (_yDiff * 0.009))];
        };
    };
    
    lilc_shops_clothing_camera camSetTarget lilc_shops_clothing_defaultPosition;
