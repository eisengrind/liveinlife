...
KRON_mod_strlen = {
    ...
    _chars = count ([_str] call KRON_StrToArray);
    
    _chars;
};

KRON_mod_generateRandomStr = {
    private["_chars","_random","_num"];
    _num = _this select 0;

    _chars = ["A","a","B","b","C","c","D","d","E","e","F","f","G","g","H","h","I","i","J","j","K","k","L","l","M","m","N","n","O","o","P","p","Q","q","R","r","S","s","T","t","U","u","V","v","W","w","X","x","Y","y","Z","z","1","2","3","4","5","6","7","8","9","0"];
    _random = "";

    for [{_i = 0},{_i < _num},{_i = _i + 1}] do {
        _rand = round (random ((count _chars ) - 1));
        _random = _random + (_chars select _rand);
    };
...
KRON_mod_trimStr = {
    ...
    if (([_str] call KRON_StrLen) <= _chars) exitWith {_str;};
    
    _strArray = [_str] call KRON_StrToArray;
    
    _str = "";
    for [{_i = 0},{_i < _chars},{_i = _i + 1}] do {
        _str = format["%1%2",_str,(_strArray select _i)];
    };
    _str = format["%1...", _str];
...
KRON_StrSplit = {
	...
	try {
		if (isNil "_string") throw [_string];
		if (isNil "_string") throw [_string];
		
		private _index = (_string find _char);
		if (_index == -1) throw [_string];

		private _firstPart = toArray _string;
		_firstPart deleteRange [_index, (count _firstPart)];
		_firstPart = toString _firstPart;

		private _lastPart = toArray _string;
		_lastPart deleteRange [0, (_index + 1)];
		_lastPart = toString _lastPart;

		throw [_firstPart, _lastPart];
	} catch {
		_exception;
	};
};
