...
try
{
	...
	do
	{
...

		{
			...
			{
				{
					if ((20000 + (_x getVariable ["lilc_uniqueObjectID", 0])) == (ctrlIDC _control)) throw true;
...

		{
			...
			private _vehicleCallSign = (_vehicle getVariable ["lilc_garage_callsign", ""]);
			...
			private _vehicleUniqueID = (_vehicle getVariable ["lilc_uniqueObjectID", 0]);
		...
		if (lilc_atcInterface_currentPlaneControl != -1) then
		{
			...
			_uiGroupInformationsTextCallsign ctrlSetText (_control getVariable ["lilc_control_callsign", ""]);
