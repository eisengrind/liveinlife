...
if (lilc_action_active) exitWith { hint "Du kannst noch nicht abbauen!"; };
...
try {
    ...
    private _stringItems = "";
    {
        ...
        if (_classname in _generatedItems) then {
            private _items = [];
            for [{_i = 0}, {_i < getNumber(_x >> "amount")}, {_i = _i + 1}] do { _items pushBack _classname; };
            [(getPosASL player), _items] call lilc_inventory_fnc_groundItems;

            private _itemConfig = ([_classname] call lilc_inventory_fnc_getItemInfo);
            _stringItems = format["%1<br /><t align='left'> - %2x %3</t>", _stringItems, getNumber(_x >> "amount"), getText(_itemConfig >> "displayName")];
        };
    } forEach ("true" configClasses _areaConfig);
    ...
    waitUntil { ((getStamina player) > (getNumber(_areaConfig >> "timeout"))) };
    sleep 1;
